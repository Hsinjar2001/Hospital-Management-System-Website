...
const Home_page = () => {
  ...
  const heroSlides = [
    {
      ...
      buttonAction: () => navigate('/auth/login') // CHANGED: from register to login
    },
    {
      id: 2,
  ...
  // Real statistics (will be fetched from API in production)
  const sampleStats = {
    totalPatients: 0, // Will be fetched from API
    doctorsCount: 0,  // Will be fetched from API
    departments: 0,   // Will be fetched from API
    yearsOfService: 15,
    successfulSurgeries: 0, // Will be fetched from API
    satisfactionRate: 0     // Will be fetched from API
  };
            ...
            {featuredDoctors.map((doctor) => (
              <div key={doctor.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <button 
                    onClick={() => navigate('/auth/login')} // CHANGED: redirect to login page
